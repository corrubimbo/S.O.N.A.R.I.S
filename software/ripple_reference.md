...
see the box below for which one you're running.
...
> **Two builds, one visualizer.** The ripple visuals are identical; only where the sound
> and metadata come from differs. Change the visuals in one, mirror them in the other.
>
> | | [`ripple_pi_windows.py`](ripple_pi_windows.py) | [`ripple_pi_linux.py`](ripple_pi_linux.py) |
> |---|---|---|
> | **Role** | desktop / development | Raspberry Pi deployment |
> | **Audio sources** | `file`, `cava` (live system audio), `sim` | `file` only |
> | **Now-playing card** | yes (Windows "now playing" via `winsdk`) | no |
> | **On-screen text** | music details only (title/artist/progress) | music details + `F` FPS overlay |
> | **Default rings (`N_BANDS`)** | 7 | 6 |
>
> **Both builds boot with no audio and no art by default** — they play a randomized
> visualizer loop over a procedural backdrop until you point them at your own files
> (see §2). This reference's exact numbers follow the 7-band Windows build; the Linux
> build is the same idea with 6 rings.
...
> the table first. **You don't need any files to start** — with nothing set up it plays a
> random ripple loop over an invented backdrop. To use your own song and cover, drop the
> files next to the script and fill in their names (see **Assets** below).

```bash
# Windows / desktop test (windowed):
python ripple_pi_windows.py

# Raspberry Pi, headless (straight to the display, no desktop):
SDL_VIDEODRIVER=kmsdrm python ripple_pi_linux.py
```
...
| `cava` | listens to live system audio (the `"cava"` source) | Windows build, live audio only |
| `winsdk` | reads Windows "now playing" title/art | optional, Windows build only |

**Assets (optional):** by default both `AUDIO_FILE` and `ALBUM_PATH` are **blank**, so the
visualizer boots with a random loop and a procedural backdrop — no files required. To use
your own, put them in the **same folder as the script** and set the names near the top:

```python
ALBUM_PATH = "cover.png"   # ← fill in for a custom image (same folder); blank = procedural backdrop
AUDIO_FILE = "song.wav"    # ← fill in for custom audio  (same folder); blank = random visualizer loop
```

The path is relative to the script, so a bare filename works (any subfolder path works too).
A missing or unreadable file just falls back to the default, so it always runs.

## 3. Controls

| Key | What it does | Build |
|---|---|---|
| `Space` | Pause/unpause. The rings settle flat and hold still; the background keeps drifting. | both |
| `F` | Show/hide the little FPS counter in the top-left corner. | **Linux only** |
| `Q` or `Esc` | Quit. | both |

> **No debug overlay on the Windows build.** It draws **only** the music details (title,
> artist, cover, progress bar) — the top-left `FPS / GRIDpx` readout and its `F` toggle were
> removed, so nothing else appears on screen. The Linux build keeps the overlay, since it's
> useful for checking you're hitting the frame budget on the Pi.
...
| Setting | Default | What it does |
|---|---|---|
| `ALBUM_PATH` | `""` (blank) | Cover image, relative to the script (same folder). Blank = a procedural gradient backdrop. |
...
> Present in both builds, but the **live** "now playing" fields (`USE_WINSDK_MEDIA`,
> `MEDIA_POLL_SEC`) are **Windows build only**. On the Linux build the title comes from the
> audio filename (or `"SONARIS"` when there's no file) and the artist from `TRACK_ARTIST`.

| Setting | Default | What it does |
|---|---|---|
| `ART_SIZE` | `160` | Size of the sharp cover square in the middle [px]. |
| `TRACK_TITLE` | `""` | Song title. Blank = use the audio filename, or `"SONARIS"` when there's no file. |
...
| Setting | Default | What it does |
|---|---|---|
| `AUDIO_SOURCE` | `"cava"` | **Windows build only.** Where sound comes from: `"file"` (a song file), `"cava"` (whatever your system is playing), or `"sim"` (the random loop). Falls back to `"sim"` if a source fails. *(Linux build has no CAVA — it uses `AUDIO_ENABLED` + `AUDIO_FILE`, and falls back to the random loop.)* |
| `AUDIO_ENABLED` | `True` | **Linux build only.** Turn the file-audio path on/off. Off (or a blank/missing file) = the random visualizer loop. |
| `BAND_EDGES` | *(edges, see §8)* | The cutoff frequencies [Hz] that split the sound into the bands. One shared split for all sources. (Windows build; the Linux build derives its 6 log-spaced edges automatically.) |
| `AUDIO_FILE` | `""` (blank) | The song to analyze, relative to the script (same folder). Blank = the random visualizer loop. |
...
| `CAVA_AUTOSTART` | `True` | *(Windows build)* Launch CAVA automatically vs. read one you started yourself. |
| `CAVA_BARS` | `32` | *(Windows build)* How many frequency bars CAVA sends, which then get grouped into the bands. |
| `CAVA_FIFO` | `/tmp/cava_fifo` | *(Windows build)* The pipe path when you run CAVA yourself. |
...
**The sources** (both builds have `"file"` and the random loop; only the Windows build has `"cava"`):
- **`"file"`** — reads the song, looks at a short slice (nudged ahead by `AUDIO_LATENCY_MS`
  ...
  parts still show.
- **`"cava"`** *(Windows build)* — a background helper listens to whatever your system is
  playing and sends frequency bars; the script groups those bars into the same 7 bands. It
  keeps the loudest reading between frames so quick beats are never missed.
- **The random visualizer loop** (the `"sim"` fallback) — the default when there's no audio
  file. A decaying-bass + shimmering-highs synthetic beat whose tempo, punch, and per-band
  timing are **randomized once at startup**, so each launch looks a little different. This is
  what plays out of the box.
...
- **CAVA's autostart config wins** *(Windows build)*. Editing `~/.config/cava/config` does
  nothing while autostart is on — change the script's `_CAVA_CFG` instead.
- **A blank or missing audio file is normal, not an error** — both builds ship with
  `AUDIO_FILE = ""` and fall straight into the random visualizer loop. Check the startup line
  in the console to see what's actually live.
- **The random loop looks different every launch** — that's intentional; its tempo and
  per-band timing are re-rolled at startup.

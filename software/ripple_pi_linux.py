...
Run (Pi, headless):   SDL_VIDEODRIVER=kmsdrm python ripple_pi.py
    Audio:  optional — boots with a random visualizer loop.  To use your own,
            drop a WAV/FLAC/OGG next to this script and set AUDIO_FILE.
...
# ─── ASSETS ───────────────────────────────────────────────────────────────────
# By default SONARIS boots with NO audio or art — it plays a random visualizer
# loop and shows a procedural backdrop.  To use your own, drop the files in the
# SAME FOLDER as this script and fill in the names below (path is relative to it).

# ─── APPLE-MUSIC BACKGROUND ───────────────────────────────────────────────────
ALBUM_PATH    = ""       # ← fill in for a custom image, e.g. "cover.png" (same folder)
...
AUDIO_FILE    = ""       # ← fill in for custom audio, e.g. "song.wav" (same folder);
                         #   blank → a random visualizer loop plays instead
AUDIO_LOOP    = True
...
# Randomised parameters for the no-audio fallback loop — rolled once at boot so
# each launch gets a different (but coherent) synthetic pattern.
_SIM_TEMPO  = random.uniform(0.45, 0.80)                                  # sec / beat
_SIM_DECAY  = random.uniform(6.0, 9.0)                                    # bass punch
_SIM_SWING  = random.uniform(0.10, 0.20)                                  # treble wobble
_SIM_RATES  = np.random.uniform(3.5, 6.5, N_BANDS).astype(np.float32)     # per-band speed
_SIM_PHASES = np.random.uniform(0.0, 2.0 * np.pi, N_BANDS).astype(np.float32)
...
def _init_audio_file() -> bool:
    ...
    try:
        ...
        return False
    if not AUDIO_FILE:
        print("[SONARIS] no audio file set — playing a random visualizer loop.")
        return False
    if not os.path.exists(AUDIO_FILE):
        print(f"[SONARIS] {AUDIO_FILE!r} not found — playing a random visualizer loop.")
        return False
    try:
...
def _audio_target(t: float) -> np.ndarray:
    ...
    # ── random visualizer loop (no / failed audio file) ─────────────────────────
    beat = (t % _SIM_TEMPO) / _SIM_TEMPO
    bass = float(np.clip(np.exp(-_SIM_DECAY * beat) * 0.9, 0, 1))
    ti   = np.linspace(0.0, 1.0, N_BANDS, dtype=np.float32)
    base = (1 - ti) * bass + ti * (0.18 + _SIM_SWING * np.abs(np.sin(t * _SIM_RATES + _SIM_PHASES)))
...
if not TRACK_TITLE:
    TRACK_TITLE = os.path.splitext(os.path.basename(AUDIO_FILE))[0] or "SONARIS"
DURATION_TOTAL = 213
if AUDIO_ENABLED and _audio_data is not None:
